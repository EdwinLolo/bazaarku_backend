  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message: "All fields except area_id and vendor_id are required",
      });
    }
    ...
    if (!files || !files.banner_image || !files.permit_img) {
      return res.status(400).json({
        success: false,
...
controller.getAllEvents = async (req, res) => {
  try {
    ...
    // Process data to add booth count
    ...
    const eventData = {
      ...
      status:
        new Date(data.end_date) < new Date()
          ...
          ? "ongoing"
          : "upcoming", // Fixed incomplete ternary
      ...
      is_registration_open: new Date(data.start_date) > new Date(), // Can still register if event hasn't started
      formatted_dates: {
